...
from query_agent.constants import DEFAULT_MIN_SCORE, DEFAULT_TOP_K
...
def rank_documents(state: dict[str, Any]) -> dict[str, Any]:
    ...
    Chunks scoring below ``min_score`` are dropped first, so an off-topic query
    returns no results instead of the nearest unrelated documents.
    """
    ...
    relevant = [row for row in raw_results if row["score"] >= min_score]
